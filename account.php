  ...
  <div id="wrapper">
    ...
    <ul class="navbar-nav bg-gradient-success sidebar sidebar-dark accordion" id="accordionSidebar">
      ...
      <a class="sidebar-brand d-flex align-items-center justify-content-center" href="account.php">
        <div class="sidebar-brand-text mx-3">UDS EXAM OFFICE </div>
      </a>
          ...
          <div class="row">
            ...
            <div class="col-xl-7 col-lg-6">
              <div class="card shadow mb-4">
                ...
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-success">Today's Courses</h6>
                </div>
                <!-- Card Body -->
                <div class="card-body">
                  <table class="table table-stripped" id="example" style="width:100%;">
                      ...
                      <tbody>
                          <?php
                                ...
                                $stmt = $conn->prepare("SELECT course_code,hall_code, startingtime FROM hallscourses WHERE writtendate = ?");
                                $stmt->bind_param("s", $date);
                                $stmt->execute();
                                $stmt->store_result();
                                $stmt->bind_result($course,$hall, $stime);

                                while($stmt->fetch()){
                                  if($stime == 1){
                                  $stime = "Morning [9:00am]";
                                }else{
                                  $stime = "Afternoon [2:00pm]";
                                }
                                  echo("<tr>
                                        <td> $course</td>
                                        <td> $stime </td>
            ...
            <div class="col-xl-5 col-lg-6">
              <div class="card shadow mb-4">
                <!-- Card Header - Dropdown -->
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-success">Exams Halls In Use Today</h6>

                </div>
                <!-- Card Body -->
                <div class="card-body">

                  <table class="table table-stripped table-sm" id="example" style="width:100%;">
                      <thead>
                          <tr class="text-white" style="background-color:#1cc88a;">
                              <th>Exam Hall </th>
                      ...
                      <tbody>
                        <tr>
                            <td colspan="3" class="text-center text-info" >Morning Session</td>
                          </tr>
                        <?php
                        $t = 1;
                        $stmt = $conn->prepare("SELECT DISTINCT hall_code FROM hallscourses WHERE startingtime = ? AND writtendate = ?");
                        $stmt->bind_param("ss",$t, $date);
                        $stmt->execute();
                        $stmt->store_result();
                        $stmt->bind_result($hall_code);
                        while($stmt->fetch()){
                          echo("
                            <tr>
                              <td> $hall_code</td>
                              <td><a class='btn btn-outline-success btn-block btn-sm rounded-pill' href='seatarrangement.php?id=$hall_code'> Generate </a></td>
                              <td><button class='btn btn-outline-danger btn-block btn-sm rounded-pill'> Generate </button></td>
                            </tr>
                            ");
                        }
                        ...
                        ?>
                        <tr>
                            <td colspan="3" class="text-center text-info" >Afternoon Session</td>
                          </tr>
                        <?php
                        $t = 0;
                        $stmt = $conn->prepare("SELECT hall_code FROM hallscourses WHERE startingtime = ? AND writtendate = ?");
                        $stmt->bind_param("ss",$t, $date);
                        $stmt->execute();
                        $stmt->store_result();
                        $stmt->bind_result($hall_code);
